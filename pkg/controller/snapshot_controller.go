...
func (ctrl *csiSnapshotController) getCreateSnapshotInput(snapshot *crdv1.VolumeSnapshot) (*crdv1.VolumeSnapshotClass, *v1.PersistentVolume, string, map[string]string, error) {
	...
	snapshotterSecretsRef, err := getSecretReferenceMany(ctrl.client, snapshotterSecretParams, class.Parameters, contentName, snapshot)
	if err != nil {
		return nil, nil, "", nil, err
	}
...
func (ctrl *csiSnapshotController) checkandUpdateBoundSnapshotStatusOperation(snapshot *crdv1.VolumeSnapshot, content *crdv1.VolumeSnapshotContent) (*crdv1.VolumeSnapshot, error) {
	...
	if snapshot.Spec.Source == nil {
		...
		driverName, snapshotID, timestamp, size, readyToUse, progressCnt, progressMsg, err = ctrl.handler.CreateSnapshot(snapshot, volume, class.Parameters, snapshotterCredentials)
		if err != nil {
	...
	newSnapshot, err := ctrl.updateSnapshotStatus(snapshot, readyToUse, timestamp, size, IsSnapshotBound(snapshot, content), progressCnt, progressMsg)
	if err != nil {
		return nil, err
	}
...
func (ctrl *csiSnapshotController) createSnapshotOperation(snapshot *crdv1.VolumeSnapshot) (*crdv1.VolumeSnapshot, error) {
	...
	for i := 0; i < ctrl.createSnapshotContentRetryCount; i++ {
		...
		newSnapshot, err = ctrl.updateSnapshotStatus(snapshot, readyToUse, timestamp, size, false, progressCnt, progressMsg)
		if err == nil {
			break
		}
...
func (ctrl *csiSnapshotController) deleteSnapshotContentOperation(content *crdv1.VolumeSnapshotContent) error {
	...
	if snapshotClassName != nil {
		if snapshotClass, err := ctrl.classLister.Get(*snapshotClassName); err == nil {
			...
			snapshotterSecretsRef, err := getSecretReferenceMany(ctrl.client, snapshotterSecretParams, snapshotClass.Parameters, content.Name, nil)
			if err != nil {
				return err
			}
			snapshotterCredentials, err = getCredentialsMany(ctrl.client, snapshotterSecretsRef)
...
func (ctrl *csiSnapshotController) updateSnapshotStatus(snapshot *crdv1.VolumeSnapshot, readyToUse bool, createdAt, size int64, bound bool, progressCnt int32, progressMsg string) (*crdv1.VolumeSnapshot, error) {
	...
	if progressCnt != 0 && progressMsg != "" {
		progress := fmt.Sprintf("%s... (%d%%)", progressMsg, progressCnt)
		ctrl.eventRecorder.Event(snapshotClone, v1.EventTypeNormal, "SnapshotProgress", progress)
	}
