...
func getSecretReference(secretParams deprecatedSecretParamsMap, snapshotClassParams map[string]string, snapContentName string, snapshot *crdv1.VolumeSnapshot) (*v1.SecretReference, error) {
...
func getCredentials(k8s kubernetes.Interface, ref *v1.SecretReference) (map[string]string, error) {
	...
	if err != nil {
		klog.Warningf("error getting secret %s in namespace %s: %v", ref.Name, ref.Namespace, err)
		return nil, nil
	}
